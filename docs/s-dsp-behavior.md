...
- [The envelope](#the-envelope)
  - [The update runs every sample; the counter gates only the store](#the-update-runs-every-sample-the-counter-gates-only-the-store)
  - [Attack ends when the step leaves the 11-bit range, not at a fixed level](#attack-ends-when-the-step-leaves-the-11-bit-range-not-at-a-fixed-level)
  - [Decay reads its sustain boundary from whichever register is driving the envelope](#decay-reads-its-sustain-boundary-from-whichever-register-is-driving-the-envelope)
  - [A decay whose level already sits in the sustain band sustains without a step](#a-decay-whose-level-already-sits-in-the-sustain-band-sustains-without-a-step)
  - [The Bent-Increase reference is saved every sample and is not clipped](#the-bent-increase-reference-is-saved-every-sample-and-is-not-clipped)
  - [The envelope is applied before it is updated](#the-envelope-is-applied-before-it-is-updated)
- [Key-on and key-off](#key-on-and-key-off)
  - [A key-on is followed by six silent samples, not five](#a-key-on-is-followed-by-six-silent-samples-not-five)
  - [Whether the silent samples advance the stream depends on the voice the key-on lands on](#whether-the-silent-samples-advance-the-stream-depends-on-the-voice-the-key-on-lands-on)
  - [The startup walk keeps no interpolation fraction](#the-startup-walk-keeps-no-interpolation-fraction)
  - [The 128 kHz ceiling is a clamp on the position, not a cap on the step](#the-128-khz-ceiling-is-a-clamp-on-the-position-not-a-cap-on-the-step)
  - [A key-on landing on a voice inside its silent span is absorbed or rewinds the silence](#a-key-on-landing-on-a-voice-inside-its-silent-span-is-absorbed-or-rewinds-the-silence)
  - [A key-on's consumption sample is shielded from a soft reset](#a-key-ons-consumption-sample-is-shielded-from-a-soft-reset)
  - [A soft reset silences the sample after the one it lands on](#a-soft-reset-silences-the-sample-after-the-one-it-lands-on)
  - [A key-on consumed on a keyed-off in-span voice restarts it in full](#a-key-on-consumed-on-a-keyed-off-in-span-voice-restarts-it-in-full)
  - [A full restart's consuming sample still emits the final pre-key-on sample](#a-full-restarts-consuming-sample-still-emits-the-final-pre-key-on-sample)
- [The intra-sample schedule](#the-intra-sample-schedule)
  - [The counter advances ahead of the checks that share its slot](#the-counter-advances-ahead-of-the-checks-that-share-its-slot)
  - [The noise steps ahead of voice 0's compute in the same slot](#the-noise-steps-ahead-of-voice-0s-compute-in-the-same-slot)
  - [The echo unit reads across five slots, not in one act](#the-echo-unit-reads-across-five-slots-not-in-one-act)
  - [The echo buffer writes land at T30/T31](#the-echo-buffer-writes-land-at-t30t31)
  - [`ESA` and `EDL` apply one sample after the write](#esa-and-edl-apply-one-sample-after-the-write)
  - [The `$F8`/`$F9` port bytes are out of the echo's reach](#the-f8f9-port-bytes-are-out-of-the-echos-reach)
  - [`VxENVX` carries an extra sample of read-back pipeline](#vxenvx-carries-an-extra-sample-of-read-back-pipeline)
  - [The pitch pair is read at the voice's own slots and steps the next advance — a key-on holds the reads for its silent span](#the-pitch-pair-is-read-at-the-voices-own-slots-and-steps-the-next-advance--a-key-on-holds-the-reads-for-its-silent-span)
  - [The BRR header is read every sample](#the-brr-header-is-read-every-sample)
  - [The decoder runs eight samples ahead of the cursor, and the check trails it by one](#the-decoder-runs-eight-samples-ahead-of-the-cursor-and-the-check-trails-it-by-one)
  - [BRR data is read in groups of four, ahead of the sound — twelve samples at the key-on](#brr-data-is-read-in-groups-of-four-ahead-of-the-sound--twelve-samples-at-the-key-on)
  - [A later group's header is read at its scheduling, its data bytes one sample later](#a-later-groups-header-is-read-at-its-scheduling-its-data-bytes-one-sample-later)
  - [The loop address is read at the voice's directory slot, ahead of the compute](#the-loop-address-is-read-at-the-voices-directory-slot-ahead-of-the-compute)
  - [The start pointer is read at the voice's directory slot, one sample after the consuming compute](#the-start-pointer-is-read-at-the-voices-directory-slot-one-sample-after-the-consuming-compute)
  - [The source number is read at its own slot, four ahead of the directory read](#the-source-number-is-read-at-its-own-slot-four-ahead-of-the-directory-read)
  - [`PMON`, `NON`, `EON` and `DIR` are read once a sample, not by the voices that use them](#pmon-non-eon-and-dir-are-read-once-a-sample-not-by-the-voices-that-use-them)
  - [Pitch modulation reads the previous voice's output from the previous sample](#pitch-modulation-reads-the-previous-voices-output-from-the-previous-sample)
  - [`ENDX` is set when the decoder leaves the end block, and reads back three slots after the compute](#endx-is-set-when-the-decoder-leaves-the-end-block-and-reads-back-three-slots-after-the-compute)
  - [The DSP's own register writes lose to a CPU write issued in the two cycles before them](#the-dsps-own-register-writes-lose-to-a-cpu-write-issued-in-the-two-cycles-before-them)
  - [The internal key-on is cleared one slot before the poll, and takes a write from the two cycles before it](#the-internal-key-on-is-cleared-one-slot-before-the-poll-and-takes-a-write-from-the-two-cycles-before-it)
- [Sample arithmetic](#sample-arithmetic)
  - [The interpolation kernel wraps on two additions and clamps on the third](#the-interpolation-kernel-wraps-on-two-additions-and-clamps-on-the-third)
  - [A shift of 13 to 15 decodes as shift 12 over the nibble shifted right three](#a-shift-of-13-to-15-decodes-as-shift-12-over-the-nibble-shifted-right-three)
- [Echo](#echo)
  - [The mix clamps at every addition, not once at the end](#the-mix-clamps-at-every-addition-not-once-at-the-end)
  - [The FIR output is 15 bits](#the-fir-output-is-15-bits)
  - [Every volume multiply is 16 bits wide and wraps](#every-volume-multiply-is-16-bits-wide-and-wraps)
