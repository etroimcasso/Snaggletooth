...
| Get the bytes the hardware received as files of their own — tile sheets as PNG, palettes, tilemaps, sprite tables and HDMA tables as text, where the run's facts name the form — with the bank file including each where it was, and a preview beside a file the run cannot turn into a source | [snes-disassembler.md §The assets](snes-disassembler.md#the-assets), [asset-formats.md](asset-formats.md), [project-manifest.md §2.12](project-manifest.md#212-assets) |
...
| [project-manifest.md](project-manifest.md) | The project manifest — the lines that name the image, the files and their ranges, the sound program's blocks, the entries, stops and warnings, the hardware the code reaches and the routines that reach it, the destinations a run reached and the places it landed, the bytes derived, the ranges a run saw the transfer engines move, where each landed and at what depth, how each HDMA table was walked, the files lifted out of the banks and the previews beside them, where every range built in work RAM came from and what was built from each file, the runs of bytes the CPU carried, what every path proves at each label and what a run saw at each site; what each tool reads back; stability |
...
| [asset-formats.md](asset-formats.md) | The asset file formats — tiles as an indexed PNG, and palettes, tilemaps, OAM and HDMA tables as text, each exact both ways; which form a lifted file takes and the previews beside the rest; and the reader that includes them back into an assembly |
