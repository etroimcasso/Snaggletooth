...
| Watch a cartridge run, play it with a controller, and record the run | [tools/player/README.md](../tools/player/README.md) |
| Plug in a controller and play a cartridge, or write an input mapping of your own | [pad-config.md](pad-config.md) |
| Find where your configs and saves are kept, or keep them somewhere else | [user-files.md](user-files.md) |
| Keep a cartridge's battery save from a program of your own | [snes-machine.md §The save observer](snes-machine.md#the-save-observer) |
| Take the picture the machine draws | [ppu.md §The picture](ppu.md#the-picture) |
| See how a line is drawn in half-pixels, and how an interlaced frame reads its half-lines | [ppu.md §The half-pixel line](ppu.md#the-half-pixel-line), [ppu.md §The interlaced picture](ppu.md#the-interlaced-picture), [ppu-behavior.md §Hires](ppu-behavior.md#hires) |
...
| Play a whole library of cartridges, none of which has a recorded run of its own | [tools/inputs/README.md](../tools/inputs/README.md) |
...
| Get the bytes the hardware received as files of their own — tile sheets as PNG, palettes, tilemaps, sprite tables and HDMA tables as text, where the run's facts name the form — with the bank file including each where it was, a preview beside a file the run cannot turn into a source, and a WAV of every sample the DSP was told to play | [snes-disassembler.md §The assets](snes-disassembler.md#the-assets), [asset-formats.md](asset-formats.md), [project-manifest.md §2.12](project-manifest.md#212-assets), [§2.20](project-manifest.md#220-samples) |
...
| [ppu-behavior.md](ppu-behavior.md) | Where the published PPU documentation is incomplete, ambiguous or wrong — the tilemap and character bases, a sprite's own wrap and the two undocumented size pairs, the schedule the two sprite passes keep and the opposite directions their counts run in, the sprite a program can put in front of every other and the one paragraph that states it three ways, the picture's edges, the brightness law, the latch flag, the window-area field values one source prints wrong, what colour math does and does not consult, direct colour's channel order, Mode 7's three — the multiplier's ports while it draws, the truncation of the transform's products, the flipped line's row — how offset-per-tile finds an entry, and what mosaic does with a size written part-way down a row and with Mode 7's two layers — and what settles each case |
| [ppu.md](ppu.md) | The PPU — the picture it draws and the frame observer it hands each one to, the backgrounds of all eight modes at their depths, the offset table modes 2, 4 and 6 read their backgrounds through, Mode 7's field read through its matrix with the second layer EXTBG makes of it, the line drawn in half-pixels and the colour math that reaches both halves of it, the interlaced picture and sprites at half height, mosaic, direct colour, and the sprites over them all in the order each mode puts them in, the two passes that find a line's sprites during the line before it under the counts the chip can afford, the converter; and the register file beneath: every write with the latches it passes through, the dot map the counter latch answers, the frame's shape under interlace and the taller picture, the two masking windows that take layers away, the sub screen and the colour math that reads it, the windows in which VRAM, the sprite table and the palette can be reached, the multiplier, the H/V counter latch, the status registers, the three open-bus values a read can answer with, the power-on state, and what the documentation leaves open |
...
| [snes-disassembler.md](snes-disassembler.md) | The cartridge disassembler, renderer and verifier — a whole cartridge traced across its banks from the vectors into its program files and manifest, and `snes_render`, which writes one source file per bank from the program file with the registers as operands, labels across files and a header per routine, the sound program captured from the machine, lifted into a program file of its own and its source written from that file, the stops a person answers with entries, what is placed, `snes_verify`, which assembles the tree back and reports the difference from the image, what the traced code reaches and which routine reaches it, the cartridge run on the machine — unattended or played from a script — so its indirect jumps' destinations become entries, every instruction it executes is lifted from its fetches and checked so that where it landed without an instruction naming it becomes an entry too and the registers seen at every site are recorded, and every range the transfer engines moved is recorded with where it came from and where it landed — the words, palette entries or sprite bytes, and what the picture used them as — every such range that begins in the image lifted into a file of its own kind — a VRAM file under `maps/` or `tiles/` where the picture used it as one, written as a tile sheet, a palette, a tilemap, a sprite table or an HDMA table where the run's facts name the form, with the depth the picture used the tiles at, the palette it held and the unit a table was walked under, and a preview beside a source the run cannot turn back — every range built in work RAM — carried out by an engine or by the CPU itself — traced to the image bytes it was built from and the routine that built it and lifted as that source, every run of bytes the CPU carried from the image lifted as the run its carrier read, every sample the run's key-ons named matched to the image and written as a WAV the machine's own decoder produced, and what every path proves: the registers at each label, the values stored, and the jump tables the bytes bound, traced past without running |
| [project-manifest.md](project-manifest.md) | The project manifest — the lines that name the image, the files and their ranges, the sound program's blocks, the entries, stops and warnings, the hardware the code reaches and the routines that reach it, the destinations a run reached and the places it landed, the bytes derived, the ranges a run saw the transfer engines move, where each landed and at what depth, how each HDMA table was walked, the files lifted out of the banks and the previews beside them, the samples the key-ons named and the WAV written of each, where every range built in work RAM came from and what was built from each file, the runs of bytes the CPU carried, what every path proves at each label and what a run saw at each site; what each tool reads back; stability |
| [input-script.md](input-script.md) | The input script — the recorded run `snes_disasm --input` replays into the controller ports and the player writes of the run it just played: its line form, frames, ports, buttons, the word for a port with no controller, refusals, the writer and its two round-trip laws, library and stability |
| [pad-config.md](pad-config.md) | The pad configuration — the plain-text file that is the player's whole input mapping: its form, the keyboard, a pad read by where its buttons sit, a family section for the one pad read by its printed letters, ports, hotkeys, refusals, library and stability |
| [user-files.md](user-files.md) | Where the player keeps a person's files — the directory the platform names and how to ask for it, the layout under it, the order a mapping is looked for in, a cartridge's battery save as the file other emulators read, what is never overwritten, and moving the lot elsewhere |
...
| [asset-formats.md](asset-formats.md) | The asset file formats — tiles as an indexed PNG, and palettes, tilemaps, OAM and HDMA tables as text, each exact both ways; which form a lifted file takes, the previews beside the rest and the WAV written of every sample the DSP was told to play; and the reader that includes them back into an assembly |
...
says so in place rather than leaving the reader to infer it, and each page's "What remains open"
lists what the documentation leaves undecided.
