...
effects, and the two hardware interrupt sequences. `tools/ir/ir_text.h` writes
one from a program and reads one back into the same program, and anything that
reads lines can read it.
...
> the file carries, and writing what was read gives the same bytes.
> `snes_lift` writes a cartridge tree's program as one.
...
A program file is UTF-8 text, one record per line. The first word of a line
names the record; its fields follow, separated by spaces. A line whose first
character is `#` is a comment, and a blank line is nothing: the writer puts one
before each region, each data run, each node — before its label, where it has
one — and each interrupt sequence, and a reader gives neither a meaning. An
effect line is the one record that does not begin with its own
name — it begins with four spaces and belongs to the node or interrupt
sequence above it.
...
count on the `image` line. An address is written `$BB:XXXX`, the bank then the
offset within it. A value is written with as many digits as it needs and no
leading zeros — `$0`, `$2C`, `$1234`, `$7E0200`.
...
The first line of every file. A reader that knows version 1 reads the rest;
...
range it covers, inclusive, within one bank. Every record after it belongs to
it until the next `region`, or until `nmi`. Within a region the labels, data
runs and nodes come in address order; at one address a label comes first, then
a data run, then a node — and where two nodes share an address (§2.6) both
follow the label. A region's records account for every byte of its range: each
byte is in a node's instruction or in a data run.

A `warning` follows its region's line and carries what the trace could not
settle there, as the manifest's [`warning`](project-manifest.md#25-stops-warnings-and-notes)
line does; the text runs to the end of the line. A region has as many as the
trace gave it, and most have none.
...
the run as two hexadecimal digits, with nothing between them, on the one line.
A run is as long as the trace left it — a table, a graphic, a whole bank — so
a line may be long.

### 2.6 Nodes

A node is one instruction at one address under one mode: a header line, then
its effects (§2.7) each on a line of its own.
...
Each effect is one line under its node, indented four spaces:
...
operands it has, separated by a comma and a space. A place is written by name —
`A`, `X`, `Y`, `S`, `D`, `PC`, `PBR`, `DBR`, `P`, `E`, `T0` to `T3` — and a flag
as `P.` and its letter: `P.N`, `P.V`, `P.M`, `P.X`, `P.D`, `P.I`, `P.Z`, `P.C`.
The register `X` and the flag `P.X`, the register `D` and the flag `P.D`, are
two words. A constant is a value, `$20`.
...
disassembled with `snes_disasm mixed.smc -o mixed --no-run --no-sound` and
written by `snes_lift mixed mixed.smc -o mixed.snagir`, begins:
...
the loop, the routine and the two interrupt handlers — with a `data` line for
each run of bytes between them, and ends with the two interrupt sequences.

## 4. Refusals

A file is read whole before anything is made of it, and a line that cannot be
read refuses the whole file, naming the line:

- a first line that is not `snagir 1`, or a version this reader does not know;
- a record whose first word this page does not name;
...
- an effect line before any node or sequence, or a step, an access kind or a
  pin on an operation that does not carry one;
- a label, a data run or a node before the first `region`, or after `nmi`;
- an `nmi` or `irq` missing, or given twice.
...
image line, and each `ProgramRegion` with its file, its range, its warnings,
...
table's and the register table's own. `renderNode` and `renderEffect` write
one node and one effect as the file does, and are what `renderProgram` calls.
...
meaning. A new record, a new field at the end of a line, or a new name may be
added, and is added to this page before a release writes it; a reader treats a
record it does not know as an error, never as a line to pass over. The version
number rises only if a record's written form has to change, and a reader given
a number it does not know stops.
