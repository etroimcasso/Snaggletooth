...
| [SNES machine](docs/snes-machine.md) | **in progress** — the bus and its region pricing, the complete beam with every per-line event at its own master offset, eight DMA/HDMA channels, the controller and APU ports, the boot handshake, the reset line; no [coprocessor](#the-coprocessors) |
| [Cartridge](docs/snes-cartridge.md) | **complete** — the header, LoROM, HiROM and ExHiROM, a copier's header read and dropped, the board — its map, its chip and its save — and where every bus address lands on it: the save windows, the lower halves a plain LoROM board repeats and the ones a coprocessor's board keeps for the chip |
...
| Public embedding API | **in progress** — a machine is built from one value, run to a cycle budget or a single access at a time, and snapshotted and restored whole; it hands over stereo frames, each finished picture, every bus access and each change to the save window through observers of their own, takes a whole controller state at once, lets a host reach into any place it holds — reading or writing the bus, the three video memories, the palette, the sprite table, the audio machine's RAM and its registers, and the register file of both processors, without spending a cycle — answers any access to a watched place before it takes effect, letting it through, preventing it, or standing a byte in its place, tells a host before a chosen instruction runs and stands a return in for the routine there, calls a routine the machine holds — in the guest's own context with its register file put back, or in a frame of the host's own — and drains audio into the caller's own buffer without allocating; the whole face is exercised together on one running cartridge and on one running sound program, and a machine with a host in the loop stays deterministic — the same state and the same trace give the same bytes. The surface is not settled and may still change |
...
https://github.com/user-attachments/assets/b300d0fc-e591-425a-a2a0-2003db7bcb64
