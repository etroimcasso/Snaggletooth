...
- **The library**, `snaggletooth::snaggletooth`: the audio unit (SPC700, APU machine, S-DSP)
  ...
  PPU), each stepped by a cycle budget the host supplies. **The PPU draws**: it resolves a pixel
  per visible dot and hands each finished frame to whoever is watching.
...
applies; and it draws the sprites — both tables, the eight size pairs, the flips, the palettes and
their four places in the mode's priority order. The sub screen, the windows and colour math, the two
sprite limits and the other screen modes are the work in front of the picture now. The cartridge
...
Mode 1 picture on screen, its sprites over it, recorded as it runs. The rest of the picture — the
two sprite limits, the sub screen, the windows and colour math, and the other screen modes —
follows in order. After it, the machine is taken through the range of cartridge types until they
boot, and
