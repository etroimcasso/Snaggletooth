...
One bank, native mode, the data bank at `$7F`. Reset builds six ranges in
...
store at a time. A fill at `$81A0` writes sixteen bytes of `$AA` to `$7F:0300`
from a constant. A transfer at `$81C0` moves thirty-two bytes from `$9300` into
`$7E:0400` through the work-RAM port; two stores at `$8300` put the first two
bytes of `$9100` into `$7E:0500` through the same port; a copy at `$8380`
moves a three-byte HDMA table from `$9400` to `$7F:0600`. Then five transfers
send `$7F:0000` and `$7F:0100` to VRAM, `$7F:0300` to OAM, and `$7E:0400` and
`$7E:0500` to VRAM, and channel 1 walks the table in `$7F:0600` to `INIDISP`
every frame while the program idles.
...
CPU wrote through the port, by the routine that stored them; a stream the CPU
carried, recorded and lifted as the bytes it carried; and a table HDMA walks
out of work RAM, lifted from its source under `hdma/`.
