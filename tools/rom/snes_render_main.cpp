// snes_render — writes a tree's source files from its program files.
//
...
// `INCBIN` lines from the manifest's facts. Where the manifest names a sound
// program it reads `apu.snagir` as well and writes the sound program's SPC700
// source file from it the same way. It reads no image and runs nothing; its
// library cannot trace or lift, so the files it writes come from the program
// files and from nowhere else. Standard output names how many files were
// written.
//
...
int main(int argc, char** argv) {
  ...
  std::cout << rendered << " files rendered from " << directory.string() << "\n";
  return 0;
}
