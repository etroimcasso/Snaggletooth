...
//
// A program file is the whole program as lines that stand alone — the image it
...
// `renderProgram` writes one from a program and what the program does not
// carry; `parseProgram` reads one back into the same program, and reading what
// was written gives the program back equal on every field. The names of the
// vocabulary are written by `opName` and the rest, and a flag is qualified by
// the register it is a bit of — `P.C` — so that the register `X` and the flag
// `X` are two words on the page.
...
// One node: a header line with its address, mnemonic, operand, length, flow,
// target, mode, measured costs and register name, then each effect indented
// under it.
